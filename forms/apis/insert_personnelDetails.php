<?php
    ...
    if(in_array($ext, $valid_extensions)) {
        ...
        $dir_img = $_SESSION['ApplicationId'].".jpeg";//.$dir_imgExt;
        $path = $path.$dir_img;
        try{
            if(mysqli_query($con,"INSERT INTO `personneldetails` (`ApplicationId`, `name`, `email`, `phone`, `address`, `dob`, `age`, `gender`, `picture`, `category`, `designation`, `department`, `university`) VALUES ('$id','$n','$m','$ph','$a','$dob','$age','$g','$dir_img','$c','$d','$dep','$u')")){
                // echo "Data inserted..!!";
            ...
            }else{echo "problem in inserting personaldetails..!!";}
        }catch(Exception $e) {
            // echo 'Message: ' .$e->getMessage();
            $q ="UPDATE `personneldetails` SET `name` = '$n', `email`='$m', `phone` = '$ph', `address`='$a', `dob`='$dob', `age`='$age', `gender`='$g', `picture`='$dir_img', `category`='$c', `designation`='$d', `department`='$dep', `university`='$u'  where `ApplicationId` = '$id'";
            ...
            $q ="UPDATE `personneldetails` SET `name` = '$n', `email`='$m', `phone` = '$ph', `address`='$a', `dob`='$dob', `age`='$age', `gender`='$g', `category`='$c', `designation`='$d', `department`='$dep', `university`='$u'  where `ApplicationId` = '$id'";
            $q1 = mysqli_query($con,$q);
            if($q1){
                $q2 = "UPDATE `reg_dir` SET `stage` = 'tab2()' where `ApplicationId` = '$id'";
                if(mysqli_query($con,$q2)){
